...
// 郵便番号を入力するコンポーネント
export default class ZigInput extends Component {
    constructor(props){
        super(props)
    ...
    // パターンに合致するかチェック
    checkValue (s) {
        const zigPattern = /^\d{3}-\d{4}$/
        return zigPattern.test(s)
    }
    // 値がユーザーにより変更された時
    handleChange (e) {
    ...
    render () {
        ...
        return (<div>
            <label>郵便番号：<br/>
                <input type='text' placeholder='郵便番号を入力' value={this.state.value} onChange={e => this.handleChange(e)}/>
                {msg}
            </label>
        </div>)
    }
